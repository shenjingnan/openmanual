...
部署前 **必须** 在 `openmanual.json` 中配置 `siteUrl`。只有配置了 `siteUrl`，OpenManual 才会启用 Next.js 的静态导出模式（`output: 'export'`），生成可部署的静态文件。

**如果没有配置 `siteUrl`，`openmanual build` 将不会生成 `out/` 目录，部署产物为空。**
...
`siteUrl` 同时用于生成正确的 sitemap 和 canonical URL。如果你使用自定义域名，请将其设置为最终域名。
...
检查是否在 `openmanual.json` 中配置了 `siteUrl`。没有 `siteUrl` 时，OpenManual 不会启用静态导出模式，构建不会生成静态文件。
...
2. 将 `openmanual.json` 中的 `siteUrl` 设置为自定义域名
3. 在托管平台配置域名（Vercel/Netlify 在项目设置中添加，GitHub Pages 在仓库设置中添加）

### 部署后样式或资源加载失败

确认 `siteUrl` 配置正确。如果站点的 base path 非根路径（如 GitHub Pages 的 `/<repo-name>/`），`siteUrl` 需要包含完整路径。
